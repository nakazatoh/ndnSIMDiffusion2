...
NS_LOG_COMPONENT_DEFINE("ndn-dumbbell-8nodes-1bottleneck");
...
main (int argc, char *argv[])
{
  ...
  topologyReader.SetFileName ("src/ndnSIM/examples/topologies/topo-dumbbell-8nodes-1bottleneck.txt");  //read topology
  topologyReader.Read ();
  ...
  consumerHelper.SetAttribute ("Frequency", StringValue ("50")); // 70 interests a second
  ...
  std::string drop_trace("drop-trace-dumbbell-8nodes-1bottleneck.txt");
  std::string rate_trace("rate-trace-dumbbell-8nodes-1bottleneck.txt");
  std::string aggregate_trace("aggregate-trace-dumbbell-8nodes-1bottleneck.txt");
  std::string app_delay_trace("app-delays-trace-dumbbell-8nodes-1bottleneck.txt");
