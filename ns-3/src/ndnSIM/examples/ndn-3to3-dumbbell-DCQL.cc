 ...
 *
 *     NS_LOG=ndn.Consumer:ndn.Producer ./waf --run=ndn-3to3-dumbbell-DCQL
 */


NS_LOG_COMPONENT_DEFINE("ndn-3to3-dumbbell-DCQL");
...
main (int argc, char *argv[])
{
  ...
  consumerHelper.SetAttribute ("Frequency", StringValue ("510")); // 510 interests a second
  ...
  std::string drop_trace("drop-trace-3to3-dumbbell-DCQL.txt");
  std::string rate_trace("rate-trace-3to3-dumbbell-DCQL.txt");
  std::string aggregate_trace("aggregate-trace-3to3-dumbbell-DCQL.txt");
  std::string app_delay_trace("app-delays-trace-3to3-dumbbell-DCQL.txt");
