...
main (int argc, char *argv[])
{
  ...
  topologyReader.SetFileName ("src/ndnSIM/examples/topologies/topo-dfcc1-node.txt");
  topologyReader.Read ();
...
/*
  std::cout << "Time" << "\t"
            ...
            << "pitsize diff" << "\n";
*/
  file.open("200204_pitsize-congestion-topo-liner-dfcc.txt",std::ios::out);
  file << "Time" << "\t"
       << "NodeId" << "\t"
       << "NodeName" << "\t"
       << "NumberOfPITEntries" << "\t"
...
//  Simulator::Schedule (Seconds (0), PeriodicStatsPrinter, Names::Find<Node>("Dst2"), Seconds (0.01));
  ...
  ndn::L3RateTracer::InstallAll("200204_rate-trace-congestion-topo-liner-dfcc.txt",Seconds (0.1));
  ndn::L3AggregateTracer::InstallAll("200204_aggregate-trace-congestion-topo-liner-dfcc.txt",Seconds (0.1));
